...
The post is the day's story, told the way the biggest build-in-public accounts tell theirs: a claim, a turn, a list, a line worth saving. Not a receipt, not a diary, not a count. Every fact in it comes from tonight's message and nothing is read from anywhere else. On Sunday the same skill writes the same post about the week, and it never adds anything up.
...
| 3 THE DRAWING | AUTO: the prompt built off the post, one fal call, `squad/posts/YYYY-MM-DD.png`. No key: one line, and the run carries on |
...
|---|---|
| no `.claude/squad-roots.md` | stop: the install (g2) did not finish; say so in one line |
| the `term day 1` row is missing or reads (none yet) | beat 0: ask it |
| `.env` has no `FAL_KEY` line | nothing stops. The first run asks once at beat 0; every later run without it prints beat 3's one line and carries on |
| neither `squad/posts/<today>.md` nor `squad/posts/<today>.png` exists | beat 1: the founder types the day |
| `<today>.png` exists and `<today>.md` does not | beat 1: the post was never said yes to. The new run's drawing overwrites the old one |
| `<today>.md` exists and `<today>.png` does not | beat 3: the drawing off the saved post when a key is there, then beat 4 for the drawing alone. No key: the one line |
| both exist and the last non-empty line before `## The input` is the signature this run would write (`Day` for a day run, `Week` for a Sunday run) | done. Print the saved post and the drawing's path, and offer the re-run in the same line: "Tonight's post is saved. Type the day again to rewrite it." A new dump runs from beat 1, and the yes at beat 4 overwrites both files |

`/bip sunday` and "write this week's post" name the week. A bare `/bip` whose effective date (the one beat 0 computed, yesterday's when the run started before 4 in the morning) falls on a Sunday is `/bip sunday`: Sunday's post is the week's post, one post, one drawing.

## The outputs (2 files a day, and 2 lines once)

1. `squad/posts/YYYY-MM-DD.md`: the post verbatim, then the raw message under `## The input`. Sunday's file holds the week's post and its signature line begins `Week`.
...
**The key.** Load `.env` the way the Proven Package does, because the terminal does not read it on its own:

```
[ -f .env ] && { set -a; . ./.env; set +a; }
```

(or `FAL_KEY=$(grep -m1 '^FAL_KEY=' .env | cut -d= -f2-)` when `.env` holds lines the terminal cannot source). An empty `FAL_KEY` after the load means the line is not there. That is not a bad key and not a stop; it is the ask below, or beat 3's one line.

**First run only.** The `term day 1` row missing or reading (none yet): ask for the date, and when `.env` has no `FAL_KEY` either, ask for the key in the same message. One message, never again:

1. What date did you post your hello? (today, if you have not yet)
2. Paste your fal key and I draw the page every night (free account at fal.ai, the key at fal.ai/dashboard/keys). Skip it and the post still comes.
...
| FAL_KEY | the key, as `FAL_KEY=<key>` in `.env` (create the file when it is not there) |

A key pasted at any later point, in the dump message or after beat 3's line, is written the same way, once, and the drawing is made that same run.

**Then the count**, from the laptop's own date: `Day N = today minus term day 1, plus 1` (the hello is Day 1), and `Week N = Day N divided by 7, rounded up`. The count does not stop at 180. A run started before 4 in the morning is the night before: use yesterday's date for the count and for the files, and say which date it used in one line.
...
Print the post once, plain, ready to copy. Open the drawing for the founder (macOS `open`, Windows `explorer`, Linux `xdg-open` on the png) and name its path in one line. Under them, the hand-off, and nothing else:
...
- Never posts, never sends, never books, never opens a platform or a browser. The post leaves through the founder's hand.
...
- One question at the gate, 2 first-run questions in one message, nothing else asked, ever. The key is asked once; after that it is one line under the post, never a question.
